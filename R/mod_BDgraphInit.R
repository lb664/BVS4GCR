...
#' a non-decomposable graph and initialises the Markov chain Monte Carlo. The R code of this function is a modified version of the 
#' BDgraph R-package (\insertCite{Mohammadi2019;textual}{BVS4GCR}). For details, see also \insertCite{Mohammadi2015;textual}{BVS4GCR} and 
#' \insertCite{Alexopoulos2021;textual}{BVS4GCR}
#'
#' @references
#' \insertAllCited{}
                            ...
                            threshold = 1e-08)
{
  ...
  result <- .C("ggm_bdmcmc_ma", as.integer(niter), as.integer(burnin),
               ...
               as.integer(b), as.integer(b_star), as.double(Ds), as.integer(niter), 
               PACKAGE = "BDgraph")))
